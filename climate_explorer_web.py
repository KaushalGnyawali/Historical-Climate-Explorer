...
# CONFIGURATION
# ═════════════════════════════════════════════════════════════════
DEFAULT_LAT, DEFAULT_LON = 56.1119, -120.7809
OM_DELAY, OM_RETRIES, OM_BACKOFF = 3.5, 4, 20.0
NP_DELAY = 1.5
IMERG_WORKERS = 8
IMERG_START_DATE = date(2000, 6, 1)
IMERG_BASE = "https://gpm1.gesdisc.eosdis.nasa.gov/opendap/GPM_L3/GPM_3IMERGDF.07"
...
def prompt_settings(args):
    ...
    if not project:
        try:
            project = (input("  📂 Project folder [climate_output]: ").strip()
                       or "climate_output")
        except Exception:
            project = "climate_output"
    ...
    if not args.start_set:
        try:
            s = input(f"  📅 Start year [1990]: ").strip()
            if s:
                start_yr = int(s)
    ...
    if do_imerg and not args.no_imerg_prompt:
        ...
        try:
            ...
            if s in ("y", "yes"):
                ...
                imerg_start = f"{int(s)}-01-01" if s else f"{im_def}-06-01"
                s = input(f"     IMERG end   [{end_yr}]: ").strip()
                imerg_end = f"{int(s)}-12-31" if s else f"{end_yr}-12-31"
            else:
                do_imerg = False
        except Exception:
            do_imerg = False
...
def download_era5land(lat, lon, start_yr, end_yr):
    ...
    max_date = (date.today() - timedelta(days=5)).strftime("%Y-%m-%d")
...
def download_power(lat, lon, start_yr, end_yr):
    ...
    for cs in range(max(start_yr, 1981), end_yr + 1, 10):
        ...
        params = {
            ...
            "start": f"{cs}0101", "end": f"{ce}1231", "format": "JSON",
        }
...
def _fetch_imerg_day(args_tuple):
    ...
    """
    d, lat_idx, lon_idx, auth, debug_dir = args_tuple
    yyyy = d.strftime("%Y")
    mm = d.strftime("%m")
    ...
    url = (
        f"{IMERG_BASE}/{yyyy}/{mm}/"
        f"3B-DAY.MS.MRG.3IMERG.{yyyymmdd}-S000000-E235959.V07B.nc4"
...
def download_imerg(lat, lon, start_yr, end_yr, raw_dir,
    ...
    de = (pd.to_datetime(ime).date() if ime
          else min(date(end_yr, 12, 31), date.today() - timedelta(days=3)))
    ...
    print(f"  📅 {ds} → {de}  ({total:,} total, {len(needed):,} new, "
    ...
    for d in needed:
        ...
        if dbg:
            dbg_left[0] -= 1
        tasks.append((d, lat_idx, lon_idx, auth, dbg))
    ...
    # ── Gap-fill retry pass ──
    # Re-attempt any days that came back NaN (network glitches, timeouts)
    ...
    if nan_dates and len(nan_dates) < len(df) * 0.5:
        ...
        retry_tasks = []
        for d in nan_dates:
            retry_tasks.append((d, lat_idx, lon_idx, auth, None))
...
def main():
    ...
    pa.add_argument("--start", type=int, default=1990)
    pa.add_argument("--end", type=int, default=datetime.now().year)
    pa.add_argument("--project", type=str, default="")
    ...
    args.lat_set = "--lat" in sys.argv
    args.start_set = "--start" in sys.argv
    args.no_imerg_prompt = args.no_imerg or "--imerg-start" in sys.argv
    args.no_eccc_prompt = args.no_eccc
